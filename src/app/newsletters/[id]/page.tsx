...
export default function EditNewsletterPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<Newsletter>(`/newsletters/${newsletterId}`)
      .then((entry) => {
        if (!entry) {
        ...
        setSelectedInterests(entry.topics ?? []);
        setCustomTopic(entry.customTopic ?? "");
        setPrimaryGoal(entry.primaryGoal ?? "");
        setFrequency(entry.frequency ?? "Daily");
        setTime(entry.time ?? "08:00 AM");
        setReadTime(entry.readTime ?? 5);
      })
      .catch((err) => {
        const message =
  ...
  const hasCustomInterests = useMemo(
    () =>
  ...
  return (
    <div className="min-h-screen bg-white pb-12 dark:bg-slate-950">
      <div className="sticky top-0 bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-10 py-4 flex items-center gap-3 z-10 dark:bg-slate-950 dark:border-slate-800">
        ...
        <h1 className="text-xl font-black text-gray-900 dark:text-gray-100">Edit Newsletter</h1>
      </div>
      ...
      <div className="max-w-[820px] w-full mx-auto px-4 sm:px-6 lg:px-10 py-8 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm font-semibold text-gray-700 mb-2 dark:text-gray-300">
                FREQUENCY
              </p>
              <select
                value={frequency}
        ...
        <button
          ...
          disabled={!email || selectedInterests.length === 0 || isSaving}
